...
from .axi4_item import *


class axi4_master_cbs(UVMCallback):
...
class axi4_master(UVMDriver):  #(axi4_item)
    ...
    def __init__(self, name, parent=None):
        ...
        self.sigs = None  # axi4_vif
        self.cfg  = None  # axi4_config
        self.tag  = "AXI4_MASTER"
    ...
    def build_phase(self, phase):
        ...
        if agent is not None:
            ...
            if (not UVMConfigDb.get(self, "", "vif", arr)):
                uvm_fatal("AXI4", "No virtual interface specified for self driver instance")
            else:
                self.sigs = arr[0]
    ...
    async def run_phase(self, phase):
        uvm_info("AXI4_MASTER", "axi4_master run_phase started", UVM_MEDIUM)

        self.sigs.ar_valid  <= 0
        self.sigs.aw_valid  <= 0
        self.sigs.w_valid   <= 0
        ...
        while True:
            ...
            tr = tr[0]
            uvm_info("AXI4_MASTER", "Driving trans into DUT: " + tr.convert2string(), UVM_MEDIUM)
    ...
    async def write(self, trans):
        uvm_info(self.tag, "Doing AXI4 write to addr " + str(trans.addr), UVM_MEDIUM)
        if trans.rw:
            # writing address on its address_read channel
            self.sigs.ar_addr <= trans.addr
            while not self.sigs.ar_ready.value:
               await self.drive_delay()
            self.sigs.ar_valid <= 1

            # wait for the transaction on read_channel
            while not self.sigs.
